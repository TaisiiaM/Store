class ProductList {
  constructor(cart) {
    this.cart = cart;
    this.container = document.querySelector('.collection');
    ...
    products.forEach((product) => {
      productListDomString += `
                  <div class="card product item">
                    <img class="card-img-top" src="img/goods/${product.image}" 
                        alt="${product.title}">
  ...
  async handleProductInfoClick(event) {
    ...
    productImg.setAttribute('src', 'img/goods/' + product.image);
    ...
    btnBuy.innerText = `₴${product.price} `;
